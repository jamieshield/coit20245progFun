...
function pptxLoad(windowjQuery) {
(function ($) {
	...
	//console.log(";;;"+settings.pptxFileUrl);
		...
		let scripts=""
            ...
            var [rslt_ary,scriptsb] = processPPTX(zip);
		scripts=scriptsb
            ...
            for (var i = 0; i < rslt_ary.length; i++) {
                switch (rslt_ary[i]["type"]) {
                    case "slide":
			// pptx note scripts prematurely evaled
			//console.log(setupHack(rslt_ary[i]["data"]));
                        $result.append(rslt_ary[i]["data"]);
			break;
			var wrapper=document.createElement('div')
                        $result.append(wrapper)
			wrapper.innerHTML=rslt_ary[i]["data"];
                        break;
                    case "pptx-thumb":
	    ...
	    // now divs are added, we can add and eval the scripts
	    setTimeout(function () {pptxToHtmlCallback(scripts)},0)
	    //$(document).find("#hiddenProgs").append(scripts)
		// return to the top  to allow better debugging
			...
			// Causes reeval of ppts notes scritps
			// Can we do withotut this?
                    //$("#" + divId + " .slideAndNote").wrapAll("<div id='all_slides_warpper' class='slides'></div>");
                }
            ...
            var trnsfrmScl = "";
            var slidesHeight = $("#" + divId + " .slide").height(); // JS moved
		    ...
		    // hack height and width
                    trnsfrmScl = 'transform:scale(' + scaleVal + '); height:'+(slidesHeight*scaleVal)+'px; width:'+(slidesWidt*scaleVal)+'px; transform-origin:top left';
            }
	    ...
	    let scripts=""
		    ...
		    //console.log(filesInfo["slides"])
                ...
                var [slideHtml,notesScripts] = processSingleSlide(zip, filename, i, slideSize);
		scripts+=notesScripts
                post_ary.push({
                    "type": "slide",
                    "data": slideHtml,
        ...
        function readXmlFile(zip, filename, isSlideContent) {
            try {
                ...
                if (isSlideContent && app_verssion <= 12) {
                    //< office2007
                    //remove "<![C ATA[ ... ]]>" tag
                    //fileContent = fileContent.replace(/< [[C ATA[[(.*?)]]]]>/g, '$1');
		    cdatare=new RegExp("<![CD"+"ATA"+"[(.*?]]>","g")
                    fileContent = fileContent.replace(cdatare, '$1');
                }
	...
	var js_slideIndex_hack=0;
		...
		//console.log("pptx resname")
		//console.log(sldFileName)
		//console.log(resName)
            var resContent = readXmlFile(zip, resName);
	    //if (resContent==null) { resContent={ 'Relationships': {'Relationships' : [] }} }
		//console.log(resContent)
		//console.log(resContent["Relationships"])
	    ...
	    let show=slideContent["p:sld"]["attrs"]["show"]
	    let hiddenSlide=show=="0"
	   if (hiddenSlide) { 
		//result="" 
		   //processNotesScripts=""
		   return ["",""]
	    ...
	    var result='<input type="radio" class="section" name="section" id="p'+ndex+'" value="p'+ndex+'" '
	    if (notCheckedHack) { 
		result+="checked" 
		    notCheckedHack=false
		} 
	    result+=' >'
	    result+='<div class="slideAndButtons" style=""><!-- slide and buttons --><label for="p'+(ndex-1)+'" class="prev">←</label><label for="p'+(ndex+1)+'" class="next">→</label><br class="buttonBr"/><div class="slideAndNote">'
	    //var result="<div class='slideAndNote'>"
            if (settings.slideMode && settings.slideType == "revealjs") {
                //var result = "<section class='slide' style='width:" + slideSize.width + "px; height:" + slideSize.height + "px;" + bgColor + "'>"
                result += "<div class='slideOuter'>"
		...
		//processNotesAll=processNotes.replace(/[[object Object]]/g,"")
		scriptsre=new RegExp("\\"+"["+"object Object\\]","g")
		processNotesAll=processNotes.replace(scriptsre,"")
		processNotesAll=cleanUnprintable(processNotesAll)
		processNotesAll=setupHack((processNotesAll))
		//if (processNotesAll.indexOf("___")>=0) {
			//console.log("pptx Found ___")
		...
		// try again non lazy
		//processNotes=processNotes.replace(/\< cript\>[\s\S.]*\<\/ cript\>/g,"")
		...
		// scripts eval-ed after divs added to dom

		processNotesScripts=processNotesAll
		//processNotesScripts=processNotesScripts.replace(/\<\/s ript\>[\s\S.]*?\<s ript\>/g,"")
		scriptsre=new RegExp("<"+"/script>[\s\S.]*?<"+"script>","g")
		processNotesScripts=processNotesScripts.replace(scriptsre,"")
		if (processNotesScripts.indexOf("<"+"script>")<0) {
		   processNotesScripts=""
		}
		//processNotesScripts=processNotesScripts.replace(/^[\s\S.]*?\<s ript\>/g,"<s ript>")
		scriptsre=new RegExp("&[\s\S.]*?<"+"script>","g")
		processNotesScripts=processNotesScripts.replace(scriptsre,"<"+"script>")

		//processNotesScripts=processNotesScripts.replace(/\<\/s ript\>[.\s\S]*$/g,"</s ript>")
		//processNotesScripts=processNotesScripts.replace(/\<\/s ript\>/g,"\n")
		//processNotesScripts=processNotesScripts.replace(/\<s ript\>/g,"\n")
		// Can't add this until div is added
		//$(document).find("#hiddenProgs").append(processNotes)

		    result+="<div class='notes'>"+processNotes+"</div>"
			// slides seem to be moved so notes need to go inside
	    result += "</div><!-- slideAndNote -->"
	    result += "</div><!-- slideAndButton -->"

            //console.log("unscripted:"+processNotes)
            //console.log("scripted:"+processNotesScripts)

	    // prepend slide with span
	    spanregex=/<span.*\/>/g
	    spans=processNotes.match(spanregex)
	    if (spans!=null) {
		result=spans[0]+result
	    }
	    spanregex=/<span.*\/span>/g
	    spans=processNotes.match(spanregex)
	    if (spans!=null) {
		result=spans[0]+result
	    }
